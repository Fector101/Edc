...
export default function EntryPage({ entries, onUpdate, onDelete }) {
  const { id } = useParams()
  ...
  if(!entry) {
    return (
      <div className="max-w-3xl mx-auto">
        <button onClick={()=>navigate('/')} className="text-indigo-600 mb-4">← Back</button>
        <p>Entry not found.</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <button onClick={()=>navigate('/')} className="text-indigo-600 mb-4">← Back to Travel Notes</button>
          ...
          <div className="flex flex-wrap gap-3">
            {(entry.photos||[]).map((src,i)=>(
              <div key={i} className="relative group">
                ...
                <button onClick={()=>removePhoto(i)} className="absolute top-1 right-1 bg-red-600 text-white rounded-full w-7 h-7 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">×</button>
              </div>
            ))}
            <div>
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e)=> handleFile(e.target.files[0])} />
              <button onClick={()=>fileRef.current.click()} className="px-3 py-2 bg-emerald-500 text-white rounded">+ Add Photo</button>
            </div>
          </div>
        </div>
        ...
        <Modal onClose={()=>setShowConfirm(false)}>
          <div>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button onClick={()=>{ onDelete(entry.id); setShowConfirm(false); }} className="px-3 py-1 bg-red-600 text-white rounded">Delete</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {showEdit && (
        <Modal onClose={()=>setShowEdit(false)}>
          <EditForm entry={entry} onSave={(patch)=>{ onUpdate(entry.id, patch); setShowEdit(false); }} onCancel={()=>setShowEdit(false)} />
        </Modal>
      )}
    </div>
  )
